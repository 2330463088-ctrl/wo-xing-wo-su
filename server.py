...
ELEMENT_TEMPLATE = PUBLIC / "templates" / "民间借贷纠纷要素式起诉状模板.docx"
PRESERVATION_TEMPLATE = PUBLIC / "templates" / "诉前财产保全申请书模板.docx"
...
def _fill_word_template(template_path: pathlib.Path, tokens_payload: dict, omit_rows: list[int] | None = None) -> bytes:
    if not template_path.exists():
        raise FileNotFoundError(f"Word模板不存在：{template_path.name}")
    source = io.BytesIO(template_path.read_bytes())
    output = io.BytesIO()
    with zipfile.ZipFile(source, "r") as src:
        document = ET.fromstring(src.read("word/document.xml"))
        for node in document.findall(f".//{WORD_NS}t"):
            value = node.text or ""
            if "{{" not in value:
                continue
            node.text = re.sub(
                r"\{\{([A-Z0-9_]+)\}\}",
                lambda match: str(tokens_payload.get(match.group(1), "") or ""),
                value,
            )
        if omit_rows:
            table = document.find(f".//{WORD_NS}tbl")
            if table is not None:
                rows = table.findall(f"{WORD_NS}tr")
                valid = sorted({int(index) for index in omit_rows if str(index).isdigit()}, reverse=True)
                for index in valid:
                    if 0 <= index < len(rows):
        ...
        with zipfile.ZipFile(output, "w", zipfile.ZIP_DEFLATED) as target:
            for item in src.infolist():
                target.writestr(item, xml if item.filename == "word/document.xml" else src.read(item.filename))
    return output.getvalue()


def _fill_element_template(tokens_payload: dict, omit_rows: list[int] | None = None) -> bytes:
    return _fill_word_template(ELEMENT_TEMPLATE, tokens_payload, omit_rows)


def _fill_preservation_template(tokens_payload: dict) -> bytes:
    return _fill_word_template(PRESERVATION_TEMPLATE, tokens_payload)
...
class PreviewHandler(BaseHTTPRequestHandler):
    ...
    def do_OPTIONS(self) -> None:
        if self.path.split("?", 1)[0] in ("/api/judge", "/api/element-complaint-docx", "/api/preservation-application-docx"):
    ...
    def do_POST(self) -> None:
        ...
        if route not in ("/api/judge", "/api/element-complaint-docx", "/api/preservation-application-docx"):
            self.send_error(404)
            return
        try:
            length = int(self.headers.get("Content-Length", "0"))
            payload = json.loads(self.rfile.read(length).decode("utf-8") or "{}")
            if route == "/api/element-complaint-docx":
                data = _fill_element_template(payload.get("tokens") or {}, payload.get("omit_rows") or [])
                filename = "民间借贷纠纷要素式起诉状.docx"
                self.send_response(200)
                ...
                self.wfile.write(data)
                return
            if route == "/api/preservation-application-docx":
                data = _fill_preservation_template(payload.get("tokens") or {})
